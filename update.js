...
const fetchSupportedVersions = async (identifier) => {
  try {
    const response = await axios.get(`https://api.ipsw.me/v4/device/${identifier}?type=ipsw`);
    const versions = response.data.firmwares.map(firmware => firmware.version);
    return versions;
  } catch (error) {
    console.error(`Error fetching supported versions for ${identifier}:`, error);
    throw error;
  }
};
